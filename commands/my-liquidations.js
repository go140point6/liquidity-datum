    ...
    try {
      ...
      if (provider === "primefi") {
        ...
        const summaryRows = Array.from(summaryByMarket.values()).map((s) => [
          `${s.collSymbol} (${s.count})`,
          `${formatNumber(s.debtTotal, 2)} ${s.debtSymbol}`,
          `${formatNumber(s.collTotal, 4)} ${s.collSymbol}`,
        ]);
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            { name: "Liquidations", value: summaryRows.length ? summaryRows.map((r) => r[0]).join("\n") : "NONE", inline: true },
            { name: "Total Debt Repaid", value: summaryRows.length ? summaryRows.map((r) => r[1]).join("\n") : "", inline: true },
            { name: "Total Coll Liquidated", value: summaryRows.length ? summaryRows.map((r) => r[2]).join("\n") : "", inline: true }
          );
      ...
      for (const r of filtered) {
        ...
        const collMeta = getLoanCollMeta(loanMetaMap, r.contract_key);
